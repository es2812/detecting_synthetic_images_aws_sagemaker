...
def test(model, test_loader, criterion, labels_ix, device, hook):
...
def train(model, train_loader, criterion, optimizer, labels_ix, device, hook):
...
def main(args):
    ...
    '''
    Create a hook for the debugger
    '''
    hook = smd.Hook.create_from_json_file()
    hook.register_module(model)
    hook.register_loss(criterion)
    
    model = train(model, train_loader, criterion, optimizer, class_index_map, device, hook)

    logger.info("Testing model")
    test(model, test_loader, criterion, class_index_map, device, hook)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--learning_rate", type=float, default=0.004035719481029806)
